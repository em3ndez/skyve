...
- Escape at the final output boundary with the context-specific helper: `OWASP.escapeHtml()` for HTML text, `OWASP.escapeJs()` for JavaScript string content, and `OWASP.escapeJsonString()` for JSON string content. Prefer the renderer, component, or JSON serializer's built-in contextual escaping when one already owns that boundary.
